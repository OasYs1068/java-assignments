...
public class GourmetCoffeeGUI extends JPanel {
			...
			try {
				if(quantity>0){
					if(currentOrder.getItem(product)==null){
						...
						currentOrder.getItem(product).setQuantity(quantity+currentOrder.getItem(product).getQuantity());
					}
					orderList.setListData(currentOrder.getItems());
				}else{
					statusTextArea.setText("Error: item quantity must at least be 1.");
				}
	...
	class RemoveListener implements ActionListener {
		...
		public void actionPerformed(ActionEvent event) {
			currentOrder = new Order();
			orderList.setListData(currentOrder.getItems());
			statusTextArea.setText("Order removed.");
	...
	class SaveSalesListener implements ActionListener {
		...
		public void actionPerformed(ActionEvent event) {
			...
			fileChooser.showSaveDialog(GourmetCoffeeGUI.this);
			File saveFile = fileChooser.getSelectedFile();
			try{
				BufferedWriter in = new BufferedWriter(new FileWriter(saveFile));
				in.write(salesFormatter.formatSales(sales));
				in.close();
